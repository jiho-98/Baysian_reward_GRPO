#!/usr/bin/env python3
"""Standalone evaluation for Bayesian AH GRPO / RLVR models."""
...
# Intentionally reuse the answer-only eval split so both experiments use identical rows.
DEFAULT_EVAL_PATH = "outputs/grpo_answer_only_qwen3b_bigmath_n4_steps200/selected_eval_metadata.jsonl"
DEFAULT_OUTPUT_DIR = "outputs/eval_bayesian_ah_grpo_compare"
DEFAULT_N4_ADAPTER_PATH = "outputs/grpo_bayesian_ah080_qwen3b_bigmath_n4_steps200/checkpoint-200"
DEFAULT_N8_ADAPTER_PATH = "outputs/grpo_bayesian_ah080_qwen3b_bigmath_n8_steps200/checkpoint-200"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate base Qwen2.5-3B-Instruct and Bayesian AH GRPO LoRA adapters."
    )
...
def maybe_add_model_specs(args: argparse.Namespace) -> list[tuple[str, str, Optional[str]]]:
    ...
    specs.append(("bayesian_ah_grpo_n4", "n4", args.n4_adapter_path))
    specs.append(("bayesian_ah_grpo_n8", "n8", args.n8_adapter_path))
    return specs
...
def build_comparison_summary(
    *,
    ...
    if evaluated:
        ...
        which_model_best = {
            "base": "base",
            "n4": "bayesian_ah_grpo_n4",
            "n8": "bayesian_ah_grpo_n8",
        }[best_stem]
...
def print_compact_table(metrics_by_stem: dict[str, dict[str, Any]]) -> None:
    ...
    display_order = [("base", "base"), ("n4", "bayesian_ah_grpo_n4"), ("n8", "bayesian_ah_grpo_n8")]
...
# CUDA_VISIBLE_DEVICES=3 python3 eval_bayesian_ah_grpo.py \
#   --model_name Qwen/Qwen2.5-3B-Instruct \
#   --eval_path outputs/grpo_answer_only_qwen3b_bigmath_n4_steps200/selected_eval_metadata.jsonl \
#   --n4_adapter_path outputs/grpo_bayesian_ah080_qwen3b_bigmath_n4_steps200/checkpoint-200 \
#   --n8_adapter_path outputs/grpo_bayesian_ah080_qwen3b_bigmath_n8_steps200/checkpoint-200 \
#   --output_dir outputs/eval_bayesian_ah_grpo_compare_100
#
# Quick test on 10 examples:
# CUDA_VISIBLE_DEVICES=3 python3 eval_bayesian_ah_grpo.py \
#   --limit 10 \
#   --output_dir outputs/eval_bayesian_ah_grpo_quick10
