...
DATASET_TAG="${DATASET_TAG:-$(slugify "${DATASET_KEY}")}"
case "${DATASET_KEY}" in
  gsm8k)
    DATASET_LABEL="${DATASET_LABEL:-GSM8K}"
    PREPARE_METADATA_SCRIPT="${PREPARE_METADATA_SCRIPT:-prepare_gsm8k_metadata.py}"
    SOURCE_DATASET_NAME="${SOURCE_DATASET_NAME:-gsm8k}"
    SOURCE_DATASET_CONFIG="${SOURCE_DATASET_CONFIG:-main}"
    ;;
  math500|math-500|math_500)
    DATASET_LABEL="${DATASET_LABEL:-MATH-500}"
    DATASET_TAG="${DATASET_TAG:-math500}"
    PREPARE_METADATA_SCRIPT="${PREPARE_METADATA_SCRIPT:-prepare_math500_metadata.py}"
    SOURCE_DATASET_NAME="${SOURCE_DATASET_NAME:-ricdomolm/MATH-500}"
    SOURCE_DATASET_CONFIG="${SOURCE_DATASET_CONFIG:-}"
    ;;
  *)
    if [[ -z "${PREPARE_METADATA_SCRIPT:-}" || -z "${DATASET_LABEL:-}" ]]; then
      echo "[ERROR] Unknown DATASET_KEY=${DATASET_KEY}. Set PREPARE_METADATA_SCRIPT and DATASET_LABEL for custom datasets." >&2
      exit 1
    fi
...
GRPO_DATASET_NAME="${GRPO_DATASET_NAME:-fixed_metadata}"

CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-3}"
PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"
SEED="${SEED:-42}"
METADATA_SETTING="${METADATA_SETTING:-full_train}"
METADATA_TRAIN_SIZE="${METADATA_TRAIN_SIZE:-3000}"
METADATA_VALID_SIZE="${METADATA_VALID_SIZE:-500}"
...
SOLVER_USE_VLLM="${SOLVER_USE_VLLM:-0}"
SOLVER_VLLM_MODE="${SOLVER_VLLM_MODE:-colocate}"
SOLVER_VLLM_GPU_MEMORY_UTILIZATION="${SOLVER_VLLM_GPU_MEMORY_UTILIZATION:-0.3}"
SOLVER_VLLM_EVAL_GPU_MEMORY_UTILIZATION="${SOLVER_VLLM_EVAL_GPU_MEMORY_UTILIZATION:-0.9}"
SOLVER_VLLM_MAX_MODEL_LENGTH="${SOLVER_VLLM_MAX_MODEL_LENGTH:-0}"
SOLVER_VLLM_TENSOR_PARALLEL_SIZE="${SOLVER_VLLM_TENSOR_PARALLEL_SIZE:-1}"
...
EXPERIMENT_ROOT="${EXPERIMENT_ROOT:-outputs/${DATASET_TAG}_full_${BACKBONE_TAG}_${MODE_TAG}}"
METADATA_DIR="${METADATA_DIR:-outputs/${DATASET_TAG}_full_train_seed${SEED}}"
...
COMPARISON_JSON="${COMPARISON_JSON:-${EXPERIMENT_ROOT}/full_baseline_comparison.json}"
COMPARISON_CSV="${COMPARISON_CSV:-${EXPERIMENT_ROOT}/full_baseline_comparison.csv}"
PIPELINE_CONFIG_PATH="${PIPELINE_CONFIG_PATH:-${EXPERIMENT_ROOT}/pipeline_launcher_config.json}"
TRAIN_DATA_LABEL="${TRAIN_DATA_LABEL:-${DATASET_LABEL} official train full}"
...
if [[ "${ANALYZER_MODEL_NAME}" != "${MODEL_NAME}" && "${ALLOW_ANALYZER_MODEL_MISMATCH:-0}" != "1" ]]; then
  echo "[ERROR] ANALYZER_MODEL_NAME must match MODEL_NAME for controlled comparisons." >&2
  echo "[ERROR] Set ALLOW_ANALYZER_MODEL_MISMATCH=1 only for an explicit ablation." >&2
  exit 1
fi
...
SOLVER_VLLM_ARGS=()
if [[ "${SOLVER_USE_VLLM}" == "1" ]]; then
  SOLVER_VLLM_ARGS+=(
    --use_vllm
    --vllm_mode "${SOLVER_VLLM_MODE}"
    --vllm_gpu_memory_utilization "${SOLVER_VLLM_GPU_MEMORY_UTILIZATION}"
    --vllm_eval_gpu_memory_utilization "${SOLVER_VLLM_EVAL_GPU_MEMORY_UTILIZATION}"
    --vllm_tensor_parallel_size "${SOLVER_VLLM_TENSOR_PARALLEL_SIZE}"
  )
  if (( SOLVER_VLLM_MAX_MODEL_LENGTH > 0 )); then
    SOLVER_VLLM_ARGS+=(--vllm_max_model_length "${SOLVER_VLLM_MAX_MODEL_LENGTH}")
  fi
fi
...
payload = {
    "dataset_key": ${DATASET_KEY@Q},
    "dataset_label": ${DATASET_LABEL@Q},
    "dataset_tag": ${DATASET_TAG@Q},
    "grpo_dataset_name": ${GRPO_DATASET_NAME@Q},
    "model_name": ${MODEL_NAME@Q},
    "analyzer_model_name": ${ANALYZER_MODEL_NAME@Q},
    "backbone_tag": ${BACKBONE_TAG@Q},
    "metadata_dir": ${METADATA_DIR@Q},
    "metadata_setting": ${METADATA_SETTING@Q},
    ...
    "seed": int(${SEED@Q}),
    "train_size": ${TRAIN_SIZE@Q},
    "eval_size": ${EVAL_SIZE@Q},
    ...
    "solver_use_vllm": ${SOLVER_USE_VLLM@Q} == "1",
    "solver_vllm_mode": ${SOLVER_VLLM_MODE@Q},
    "solver_vllm_gpu_memory_utilization": float(${SOLVER_VLLM_GPU_MEMORY_UTILIZATION@Q}),
    "solver_vllm_eval_gpu_memory_utilization": float(${SOLVER_VLLM_EVAL_GPU_MEMORY_UTILIZATION@Q}),
    "solver_vllm_max_model_length": int(${SOLVER_VLLM_MAX_MODEL_LENGTH@Q}),
    "solver_vllm_tensor_parallel_size": int(${SOLVER_VLLM_TENSOR_PARALLEL_SIZE@Q}),
    ...
    "sft_val_ratio": float(${SFT_VAL_RATIO@Q}),
    "sft_evidence_fraction": float(${SFT_EVIDENCE_FRACTION@Q}),
    "sft_clean_fraction": float(${SFT_CLEAN_FRACTION@Q}),
    "sft_hard_case_top_fraction": float(${SFT_HARD_CASE_TOP_FRACTION@Q}),
    "dpo_val_ratio": float(${DPO_VAL_RATIO@Q}),
    "dpo_hard_case_top_fraction": float(${DPO_HARD_CASE_TOP_FRACTION@Q}),
...
require_file "${PREPARE_METADATA_SCRIPT}"
...
require_file "collect_full_baseline_results.py"

echo "[INFO] Common full-baseline pipeline"
echo "[INFO] dataset_key=${DATASET_KEY}"
echo "[INFO] dataset_label=${DATASET_LABEL}"
echo "[INFO] grpo_dataset_name=${GRPO_DATASET_NAME}"
...
echo "[INFO] solver_use_vllm=${SOLVER_USE_VLLM}"
...
PREPARE_CMD=(
  python3 "${PREPARE_METADATA_SCRIPT}"
  --setting "${METADATA_SETTING}"
  --output_dir "${METADATA_DIR}"
  --train_size "${METADATA_TRAIN_SIZE}"
  --valid_size "${METADATA_VALID_SIZE}"
  --seed "${SEED}"
)
if [[ -n "${SOURCE_DATASET_NAME}" ]]; then
  PREPARE_CMD+=(--dataset_name "${SOURCE_DATASET_NAME}")
fi
if [[ -n "${SOURCE_DATASET_CONFIG}" ]]; then
  PREPARE_CMD+=(--dataset_config "${SOURCE_DATASET_CONFIG}")
fi
if [[ "${DRY_RUN}" == "1" ]]; then
  PREPARE_CMD+=(--dry_run)
fi
...
if should_skip_step "${METADATA_DIR}/metadata_summary.json" "${METADATA_DIR}/selected_train_metadata.jsonl" "${METADATA_DIR}/selected_test_metadata.jsonl"; then
  ...
  run_cmd_logged "${STEP_LOG_ROOT}/01_prepare_metadata.log" "${PREPARE_CMD[@]}"
fi
  ...
  if should_skip_step "${BASE_OUTPUT_DIR}/summary.json" "${BASE_OUTPUT_DIR}/test/summary.json"; then
    ...
    run_cmd_logged \
      ...
      env CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES}" \
        ...
        USE_VLLM="${SOLVER_USE_VLLM}" \
        VLLM_EVAL_GPU_MEMORY_UTILIZATION="${SOLVER_VLLM_EVAL_GPU_MEMORY_UTILIZATION}" \
        VLLM_MAX_MODEL_LENGTH="${SOLVER_VLLM_MAX_MODEL_LENGTH}" \
        VLLM_TENSOR_PARALLEL_SIZE="${SOLVER_VLLM_TENSOR_PARALLEL_SIZE}" \
        SEED="${SEED}" \
        TRAIN_DATA_LABEL="0" \
        METHOD_NAME="Base prompted evaluation" \
        SUMMARY_NOTES="deterministic official ${DATASET_LABEL} full-test evaluation; ${PROMPT_FORMAT_NOTE}" \
  ...
  if should_skip_step "${SFT_GRPO_OUTPUT_DIR}/summary.json" "${SFT_GRPO_OUTPUT_DIR}/test/summary.json"; then
    ...
    run_cmd_logged \
      ...
      python3 run_grpo_bayesian_with_learned_analyzer.py \
        ...
        --notes "deterministic official ${DATASET_LABEL} full-test evaluation; ${PROMPT_FORMAT_NOTE}" \
        "${SOLVER_USE_LORA_ARGS[@]}" \
        "${SOLVER_GRADIENT_CHECKPOINTING_ARGS[@]}" \
        "${SOLVER_VLLM_ARGS[@]}" \
        "${BF16_ARGS[@]}"
...
if [[ "${RUN_COLLECT_RESULTS}" == "1" ]]; then
  run_cmd_logged \
    ...
    python3 collect_full_baseline_results.py \
...
echo "[DONE] Common full-baseline pipeline resolved under ${EXPERIMENT_ROOT}"
