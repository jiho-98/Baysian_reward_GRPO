...
def extract_json_object_text(text: str) -> str:
    r"""Extract the outermost JSON object candidate from model output.

    This function intentionally does not modify backslashes. It only removes
    common wrappers such as markdown fences and trims prefix/suffix text.
    """
...
def _escape_invalid_backslashes_inside_json_strings(text: str) -> str:
    r"""Escape invalid JSON backslashes without corrupting valid JSON escapes.

    Model judge outputs occasionally include LaTeX fragments inside JSON string
    values, for example:
        "\(2^5\)", "\binom{5}{2}", "\frac{1}{2}"

    In strict JSON, \(...\), \binom, \frac, etc. are invalid or can be
    misinterpreted as control escapes. This state-machine only rewrites
    backslashes while inside JSON strings. It keeps already valid JSON escapes
    such as \", \\, \/, \n, \t, and \uXXXX, but converts invalid single
    backslashes to escaped backslashes.
    """
    out: list[str] = []
    in_string = False
    i = 0
    ...
    while i < len(text):
        ...
        # We are inside a string and at one or more backslashes.
        if i + 1 >= len(text):
            out.append("\\\\")
            i += 1
            continue
        ...
        # Already escaped backslash or escaped quote/slash: keep as-is.
        if next_ch in ['"', "\\", "/"]:
            out.append("\\")
            out.append(next_ch)
            i += 2
        ...
        # Unicode escape: keep only if it has four hex digits.
        if next_ch == "u":
        ...
        # Standard control escapes. If the following character continues an
        # alphabetic command such as \binom, \frac, \times, \right, or \text,
        # treat it as a LaTeX command and escape the backslash instead.
        ...
        # Any other single backslash inside a string is invalid JSON.
        out.append("\\\\")
        i += 1
...
def repair_json_text(text: str) -> str:
    cleaned = extract_json_object_text(text)
    cleaned = re.sub(r",\s*([}\]])", r"\1", cleaned)
    cleaned = _escape_invalid_backslashes_inside_json_strings(cleaned)
    return cleaned
...
def safe_json_parse(text: str) -> Optional[Any]:
    ...
    # Try the repaired candidate first because raw JSON may technically parse
    # while silently interpreting LaTeX commands such as \binom as control
    # escapes. The repaired candidate preserves such strings as literal text.
    candidates = [
        repaired,
        extracted,
        raw,
