#!/usr/bin/env bash
set -euo pipefail

GPU_ID="${1:?usage: $0 <gpu_id> <queue_name>}"
QUEUE_NAME="${2:?usage: $0 <gpu_id> <queue_name>}"
FREE_MEM_THRESHOLD_MB="${FREE_MEM_THRESHOLD_MB:-20000}"
POLL_SECONDS="${POLL_SECONDS:-300}"

ROOT_DIR="/home/kimjh/Baysian_reward_GRPO"
cd "$ROOT_DIR"

PYTHON_BIN="${PYTHON_BIN:-.venv/bin/python}"
STACK_FILE="${STACK_FILE:-RUNNING_TRAINING_STACK.md}"
RUN_ROOT="outputs/drgrpo_answer_only_24h"

timestamp() {
  date +"%Y-%m-%d %H:%M:%S %Z"
}

gpu_used_mb() {
  nvidia-smi --id="$GPU_ID" --query-gpu=memory.used --format=csv,noheader,nounits | tr -d ' '
}

wait_for_gpu() {
  while true; do
    local used
    used="$(gpu_used_mb)"
    if [ "$used" -lt "$FREE_MEM_THRESHOLD_MB" ]; then
      echo "[$(timestamp)] GPU${GPU_ID} available: ${used}MiB used < ${FREE_MEM_THRESHOLD_MB}MiB"
      return 0
    fi
    echo "[$(timestamp)] GPU${GPU_ID} busy: ${used}MiB used; waiting ${POLL_SECONDS}s"
    sleep "$POLL_SECONDS"
  done
}

append_stack_start() {
  local job_name="$1"
  local output_dir="$2"
  local train_metadata="$3"
  local eval_metadata="$4"
  local train_size="$5"
  local max_prompt_length="$6"
  mkdir -p "$(dirname "$STACK_FILE")"
  cat >> "$STACK_FILE" <<EOF

## Train - ${job_name}

- Added at: $(timestamp)
- Status: running
- Server: PRO6000 / GPU${GPU_ID}
- Script: \`Answer_only_GRPO.py\`
- Method: Dr.GRPO external baseline
- Reward: answer-only correctness
- loss_type: \`dr_grpo\`
- scale_rewards: \`none\`
- beta: \`0.0\`
- output_dir: \`${output_dir}\`
- train_metadata_path: \`${train_metadata}\`
- eval_metadata_path: \`${eval_metadata}\`
- train_size: ${train_size}
- eval_size: 0
- num_generations: 8
- max_steps: 1500
- per_device_train_batch_size: 8
- gradient_accumulation_steps: 1
- learning_rate: 5e-6
- max_prompt_length: ${max_prompt_length}
- max_completion_length: 1024
- LoRA: r16 / alpha32 / dropout0.05
- bf16: true
- gradient_checkpointing: true
- seed: 42
- log: \`${output_dir}/logs/train.nohup.log\`
EOF
}

append_stack_finish() {
  local job_name="$1"
  local output_dir="$2"
  local status="$3"
  local started_at="$4"
  local finished_at
  finished_at="$(timestamp)"
  local checkpoints
  checkpoints="$(find "$output_dir" -maxdepth 1 -type d -name 'checkpoint-*' -printf '%f ' 2>/dev/null | xargs || true)"
  local final_checkpoint
  final_checkpoint="$(find "$output_dir" -maxdepth 1 -type d -name 'checkpoint-*' -printf '%f\n' 2>/dev/null | sort -V | tail -1 || true)"
  cat >> "$STACK_FILE" <<EOF

### Finish - ${job_name}

- Finished at: ${finished_at}
- Started at: ${started_at}
- Status: ${status}
- output_dir: \`${output_dir}\`
- final_checkpoint: \`${final_checkpoint:-none}\`
- checkpoint_list: ${checkpoints:-none}
EOF
}

run_drgrpo_job() {
  local job_name="$1"
  local model_name="$2"
  local train_metadata="$3"
  local eval_metadata="$4"
  local train_size="$5"
  local max_prompt_length="$6"
  local output_dir="$7"

  mkdir -p "$output_dir/logs"
  local started_at
  started_at="$(timestamp)"
  append_stack_start "$job_name" "$output_dir" "$train_metadata" "$eval_metadata" "$train_size" "$max_prompt_length"
  echo "[$started_at] START ${job_name} on GPU${GPU_ID}"

  set +e
  CUDA_VISIBLE_DEVICES="$GPU_ID" "$PYTHON_BIN" Answer_only_GRPO.py \
    --model_name "$model_name" \
    --dataset_name fixed_metadata \
    --use_fixed_metadata \
    --train_metadata_path "$train_metadata" \
    --eval_metadata_path "$eval_metadata" \
    --train_size "$train_size" \
    --eval_size 0 \
    --num_generations 8 \
    --max_prompt_length "$max_prompt_length" \
    --max_completion_length 1024 \
    --temperature 0.7 \
    --top_p 0.95 \
    --max_steps 1500 \
    --per_device_train_batch_size 8 \
    --gradient_accumulation_steps 1 \
    --learning_rate 5e-6 \
    --lora_r 16 \
    --lora_alpha 32 \
    --lora_dropout 0.05 \
    --min_solve_rate 0.0 \
    --max_solve_rate 1.0 \
    --seed 42 \
    --logging_steps 10 \
    --save_steps 100 \
    --progress_interval_percent 10 \
    --bf16 \
    --use_lora \
    --gradient_checkpointing \
    --loss_type dr_grpo \
    --scale_rewards none \
    --beta 0.0 \
    --output_dir "$output_dir" \
    > "$output_dir/logs/train.nohup.log" 2>&1
  local exit_code=$?
  set -e

  if [ "$exit_code" -eq 0 ]; then
    append_stack_finish "$job_name" "$output_dir" "completed" "$started_at"
  else
    append_stack_finish "$job_name" "$output_dir" "failed_exit_${exit_code}" "$started_at"
    return "$exit_code"
  fi
}

case "$QUEUE_NAME" in
  gpu0_1p7b_bigmath_then_gsm8k)
    wait_for_gpu
    run_drgrpo_job \
      "Qwen3-1.7B Dr.GRPO Answer-only BigMath BARL-style" \
      "Qwen/Qwen3-1.7B" \
      "outputs/bigmath_barl_style_12x1024_seed42/selected_train_metadata.jsonl" \
      "outputs/eval_benchmarks/olympiadbench_metadata.jsonl" \
      12288 \
      2048 \
      "${RUN_ROOT}/qwen3_1p7b_bigmath_steps1500_n8_bsz8_acc1"
    wait_for_gpu
    run_drgrpo_job \
      "Qwen3-1.7B Dr.GRPO Answer-only GSM8K" \
      "Qwen/Qwen3-1.7B" \
      "outputs/gsm8k_full_train_seed42/selected_train_metadata.jsonl" \
      "outputs/gsm8k_full_train_seed42/selected_valid_metadata.jsonl" \
      7473 \
      1024 \
      "${RUN_ROOT}/qwen3_1p7b_gsm8k_steps1500_n8_bsz8_acc1"
    ;;
  gpu1_1p7b_math500_then_4b_gsm8k)
    wait_for_gpu
    run_drgrpo_job \
      "Qwen3-1.7B Dr.GRPO Answer-only MATH-500" \
      "Qwen/Qwen3-1.7B" \
      "outputs/math500_experiments/metadata_fulltrain12000_test500_seed42/selected_train_metadata.jsonl" \
      "outputs/math500_experiments/metadata_fulltrain12000_test500_seed42/selected_test_metadata.jsonl" \
      12000 \
      2048 \
      "${RUN_ROOT}/qwen3_1p7b_math500_steps1500_n8_bsz8_acc1"
    wait_for_gpu
    run_drgrpo_job \
      "Qwen3-4B Dr.GRPO Answer-only GSM8K" \
      "Qwen/Qwen3-4B" \
      "outputs/gsm8k_full_train_seed42/selected_train_metadata.jsonl" \
      "outputs/gsm8k_full_train_seed42/selected_valid_metadata.jsonl" \
      7473 \
      1024 \
      "${RUN_ROOT}/qwen3_4b_gsm8k_steps1500_n8_bsz8_acc1"
    ;;
  *)
    echo "Unknown queue: ${QUEUE_NAME}" >&2
    exit 2
    ;;
esac
