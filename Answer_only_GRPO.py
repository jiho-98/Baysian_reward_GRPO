...
def load_dataset_rows(args: argparse.Namespace, tokenizer: Any, output_dir: Path):
    ...
    total_needed = min(after_solve_rate, args.train_size + args.eval_size)
    if total_needed < args.train_size + args.eval_size:
        print(
            f"[WARN] Filtered dataset has only {after_solve_rate} rows, "
            f"smaller than requested train+eval size {args.train_size + args.eval_size}."
        )
    ...
    train_rows = selected_rows[: min(args.train_size, len(selected_rows))]
    eval_start = len(train_rows)
    eval_rows = selected_rows[eval_start : eval_start + args.eval_size]
