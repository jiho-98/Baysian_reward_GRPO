#!/usr/bin/env python3
"""Collect full-baseline math-reasoning experiment results into JSON and CSV."""
...
DEFAULT_ROOT = "outputs/gsm8k_full_qwen3_1p7b_lora"
DEFAULT_METADATA_DIR = "outputs/gsm8k_full_train_seed42"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Collect common full-baseline comparison rows for a dataset/backbone run."
    )
    ...
    parser.add_argument("--expected_model_name", default="")
...
def metadata_label(metadata_summary: dict[str, Any] | None, fallback: str) -> str:
    if fallback:
        return fallback
    if metadata_summary is None:
        return "dataset"
    dataset_name = str(metadata_summary.get("dataset_name", "") or "").strip()
...
def collect_row(
    *,
    ...
    if root_summary is not None:
        ...
        gradient_checkpointing = root_summary.get("gradient_checkpointing", gradient_checkpointing)
    ...
    row = {
        ...
        "Suspicious Final Answer Rate": optional_float(test_summary, "suspicious_final_answer_rate"),
    ...
    if expected_model_name and row["Model"] is not None and row["Model"] != expected_model_name:
...
def main() -> None:
    ...
    metadata_summary = (
        load_json_if_exists(metadata_dir / "metadata_summary.json")
        or load_json_if_exists(metadata_dir / "summary.json")
    )
    dataset_label_value = metadata_label(metadata_summary, args.dataset_label)
    train_data_full = f"{dataset_label_value} official train full"

    output_json = Path(args.output_json) if args.output_json else root_dir / "full_baseline_comparison.json"
    output_csv = Path(args.output_csv) if args.output_csv else root_dir / "full_baseline_comparison.csv"

    rows = [
        collect_row(
            dataset_label_value=dataset_label_value,
            ...
            notes=f"deterministic official {dataset_label_value} full-test evaluation",
        ...
        collect_row(
            ...
            diagnostics_path=None,
            metadata_summary=metadata_summary,
            notes=f"official {dataset_label_value} full-train/full-test",
            expected_model_name=args.expected_model_name,
        ),
        collect_row(
            dataset_label_value=dataset_label_value,
        ...
        collect_row(
            ...
            metadata_summary=metadata_summary,
            notes="SFT analyzer built from prompted debug logs in the same run",
            expected_model_name=args.expected_model_name,
        ),
        collect_row(
            dataset_label_value=dataset_label_value,
            ...
            metadata_summary=metadata_summary,
            notes="DPO analyzer initialized from the SFT adapter in the same run",
            expected_model_name=args.expected_model_name,
        ),
    ]
    ...
    payload = {
        ...
        "expected_model_name": args.expected_model_name or None,
        "rows": rows,
    }
